...
namespace Innocellence.WeChat.Domain.Service
{
    public class SmartPhoneMenuService : BaseService<Category>, ISmartPhoneMenuService
    {
        ...
        {
            var categories = Infrastructure.Web.Domain.Service.CommonService.lstCategory.Where(x => x.AppId == appId && (x.CategoryCode == null || !x.CategoryCode.EndsWith(_endFix, true, CultureInfo.CurrentCulture)) && (x.IsDeleted == null || x.IsDeleted == false)).OrderBy(x => x.CategoryOrder).Select(x =>
            {
        ...
        {
            var category = Infrastructure.Web.Domain.Service.CommonService.lstCategory.FirstOrDefault(x => x.Id == menuId) ?? Repository.GetByKey(menuId);
            if (category == null)
            {
                return null;
            }
        ...
        public int UpdateOrAdd(AppMenuView menu)
        {
            ...
            else
            {
                ...
                {
                    var menuGroup =
                           Infrastructure.Web.Domain.Service.CommonService.lstCategory.Where(x => x.ParentCode == menu.Id || x.Id == menu.Id).ToList();
