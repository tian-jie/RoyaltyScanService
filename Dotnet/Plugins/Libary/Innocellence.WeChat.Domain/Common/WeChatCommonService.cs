...
namespace Innocellence.WeChat.Domain.Common
{
    public partial class WeChatCommonService : Innocellence.WeChat.Domain.Contracts.ICommonService
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static SysWechatConfig GetWechatConfigByAccountId(int accountId)
        {
            var weChatConfig = lstSysWeChatConfig.FirstOrDefault(x => x.Id == accountId);
            if (weChatConfig == null)
            {
                Logger.Error(string.Format("Can not find syswechat config id:{0}", accountId));
                return null;
            }
        ...
        /// <returns></returns>
        public static int GetWechatIdByAccountId(int accountId)
        {
            var wechat = GetWechatConfigByAccountId(accountId);
            if (wechat == null)
            {
                return 0;
            }

            return wechat.AccountManageId.GetValueOrDefault();
        }
