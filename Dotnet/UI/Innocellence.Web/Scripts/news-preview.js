...
app.controller('NewsEdit', function ($scope, $window, $timeout) {
    ...
    $scope.AddNewItem = function () {
        ...
        $scope.news.push({ id: $scope.id, msgtitle: '', msgfileImg: '', msgtips: '', msgmaintext: '', btntext: 'Add Comment', IsUseComment: true, showLike: "true", showReadCount: "true", showComment: "false", isWatermark: "true", securityLevel: "0", newMaterialId: 0, articleNoShare: "true" });
    ...
    $scope.firstEditLoad = false;
    //for edit

    $scope.isShowPreview = function () {
        return $scope.firstitem.msgfileImg != null && $scope.firstitem.msgfileImg.length > 0;
    };

    $scope.$watch('firstitem.msgmaintext', function (newVal) {
        jQuery('#editor1').val(newVal);
    });

    $scope.$watch('msgmaintext', function (newVal) {
        jQuery('#editor2').val(newVal);
    });

    $timeout(function () {
        if ($window.entity.Messages) {
            var firstItem = $window.entity.Messages[0];
            var editContent = JSON.parse(firstItem.ArticleContentEdit)[0];

            if (firstItem.ContentType == 2) {
                $scope.firstEditLoad = true;
                var first = $window.entity.Messages.shift();
                $scope.firstitem = { id: first.Id, InGroupOrderId: first.OrderID, msgtitle: first.ArticleTitle, msgfileImg: first.ImageCoverUrl, msgtips: first.ArticleComment, msgmaintext: first.ArticleContent, btntext: 'Add Comment', IsUseComment: true, showLike: first.isLike, showReadCount: first.ShowReadCount, showComment: first.ShowComment, isWatermark: first.IsWatermark, securityLevel: first.SecurityLevel, newMaterialId: 0, articleNoShare: first.NoShare, articleCode: first.ArticleCode };

                angular.forEach($window.entity.Messages, function (value) {
                    $scope.isAddFunGo = true; //for item repeat done;
                    //$scope.id++;
                    ...
                    $scope.news.push({ InGroupOrderId: value.OrderID, id: value.Id, msgtitle: value.ArticleTitle, msgfileImg: value.ImageCoverUrl, msgtips: value.ArticleComment, msgmaintext: value.ArticleContent, btntext: 'Add Comment', IsUseComment: true, showLike: value.ShowLikeCount, showReadCount: value.ShowReadCount, showComment: value.ShowComment, isWatermark: value.IsWatermark, securityLevel: value.SecurityLevel, newMaterialId: 0, articleNoShare: value.NoShare, articleCode: value.ArticleCode });
                    var second = angular.copy($scope.news[0]);

                    $scope.id = second.id;
                    $scope.activeId = second.InGroupOrderId - 1;
                    $scope.activeTitle = "";
                    $scope.msgtitle = second.msgtitle;
                    $scope.msgfileImg = second.msgfileImg;
                    $scope.msgtips = second.msgtips;
                    $scope.msgmaintext = second.msgmaintext;
                    $scope.btntext = second.btntext;
                    $scope.showLike = second.showLike;
                    $scope.showReadCount = second.showReadCount;
                    $scope.articleNoShare = second.articleNoShare;
                    $scope.showComment = second.showComment;
                    $scope.isWatermark = second.isWatermark;
                    $scope.securityLevel = second.securityLevel;
                });

                $scope.activeFirst();
            } else if (firstItem.ContentType == 5) {
                $scope.type4_msgVideoTitle = firstItem.ArticleTitle;
                $scope.type4_msgVideoDesc = firstItem.ArticleComment;
                $scope.type4_msgVideoSrc = editContent.VideoContent;
            } else if (firstItem.ContentType == 6) {
                $scope.type5_SoundSrc = editContent.SoundSrc;
                $scope.audioIcon = '/images/mod-icon-audio_6cb9e065.png';
                $scope.audioName = editContent.NewsTitle;
            }

            //repeat - item
            // $('.repeat-item:eq(0) a').trigger('click');
        }
...
app.directive('item', function () {
    var repeatItemHtml = "<li class='repeat-item' ng-repeat='new in news'><div class='order' style='display:none;'><b class='arrow fa fa-angle-up' ng-click='upRecord($index)'></b>&nbsp;&nbsp;<b class='arrow fa fa-angle-down' ng-click='downRecord($index)'></b></div><a href='javascript:void(0)' ng-click='active($index,$last)'>" +//repeat-done='active($index,$last)' 
        "<div class='repeat-img-box' ng-if='new.msgfileImg.length>0' ><img ng-src='{{new.msgfileImg}}'/></div>" +
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.active = function (index, last) {
                //if (scope.firstEditLoad === true && index) {
                //return;
                //}
