...
$(function () {
    ...
    var textImgJson = function textImgJson(postType) {
        ...
        var _newFirst = {
            ...
            Id: _firstNews.id,
            NewsCode: _firstNews.articleCode,
            InGroupOrderId: _firstNews.InGroupOrderId
        };
        ...
        $.each(_oldObj, function (idx, obj) {
            var _curr = {
                ...
                Id: obj.id,
                NewsCode: obj.articleCode,
                InGroupOrderId: obj.InGroupOrderId
            };
            _newsObj.push(_curr);
        });
        return _newsObj;
    ...
    //推送：1，定时：2，save：3,预览：4
    /*** msg send partial ***/
    //submitfun
    function subyulan(postType) {
        ...
        switch (msgtype) {
            case "1":
                ...
                if (_text) {
                    ...
                    _textJson[0].Id = getkey();
                    //var url = "SendNews";
                    //url = postType==='isyulan' ? url + '?isPreview=true' : url;
                    $.ajax({
                        url: url,
                        type: "post",
...
$(function () {
    ...
    $('#count').html(pattern);
    $('#saytext').on('keyup blur', function () {
    ...
    if (window.entity.Messages && window.entity.Messages.length>0) {
        var firstRow = window.entity.Messages[0];
        var type = firstRow['ContentType'] - 1;
        $('#msgType').find('li:eq(' + type + ')').trigger('click');
        $('#msgType li:not(.active)').off('click');

        var usersString = JSON.stringify(window.entity.TargetUsers);
        $('#mainGroup').val(usersString);

        showSelectedUser(window.entity.TargetUsers);
        $('#key').val(firstRow.Id);
        switch (firstRow.ContentType) {
            case 1:
                $('#saytext').val(firstRow.ArticleContent).trigger('keyup').trigger('input');
               // $('#key').val(firstRow.Id);
                break;
            case 3:
                //$('#key').val(firstRow.Id);
                $('#image3-src').val(firstRow.ImageCoverUrl).trigger('change').next().find('img').attr('src', firstRow.ImageCoverUrl);
                break;
            case 4:
                //$('#key').val(firstRow.Id);
                var row = JSON.parse(firstRow.ArticleContentEdit)[0];
                var file = row.FileSrc, fileType = file.split('.')[1];
                $('#image6-src').val(file).trigger('change');
                $('#image6-src_type').val(fileType).trigger('change');
                $('#image6-src_title').val(firstRow.ArticleTitle).trigger('change');
                $('#image6-src_size').val(row.Size).trigger('change');

                var $container = $('.mod-send-message_contentwrap');
                $container.find('.icon_filetype').addClass('filetype_' + fileType);
                $container.find('.mod-send-message_process-name').text(row.ArticleTitle).
                $container.find('.mod-send-message_process-size').text(row.Size);
                $container.find('input[type=hidden]').val(file).trigger('change');
                break;
            default:
        }
    }

    function showSelectedUser(targetUsers) {
        var outSide = "";
        $.each(targetUsers, function (i, user) {
            ...
            outSide += "<span data-gtype=" + user.Type + " data-gid=" + user.WeixinId + "><img src='" + user.Avatar + "'>" + user.WeixinName + "<i class='ace-icon glyphicon glyphicon-remove'></i></span>";
        });
