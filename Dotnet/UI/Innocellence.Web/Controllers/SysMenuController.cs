...
namespace Innocellence.Web.Controllers
{
    public class SysMenuController : BaseController<SysMenu, SysMenuView>
    {
        ...
        {
            bool needAllMenu = true;
            //bool.TryParse(Request["needAllMenu"], out needAllMenu);
            var hasPermissionNode = GetHasPermissionMenuList(needAllMenu);
        ...
        private List<SysMenu> GetHasPermissionMenuList(bool needAllMenu = false)
        {
            List<SysMenu> hasPermissionNode = null;
            if (needAllMenu)
            {
                //Expression<Func<SysMenu, bool>> predicate = FilterHelper.GetExpression<SysMenu>();
                //predicate = predicate.AndAlso(a => a.IsDeleted == false);
                //return _objService.GetList<SysMenuView>(1000, predicate).OrderBy(a => a.SortCode).ToList();
                return _BaseService.Repository.Entities.Where(a => a.IsDeleted != true).ToList();
            }
            else
            {
                var user = (Session["UserInfo"] as SysUser);
                var currentAccounManageId = int.Parse(Request.Cookies["AccountManageid"].Value);
                var currentApps = WeChatCommonService.lstSysWeChatConfig.Where(x => x.AccountManageId == currentAccounManageId).Select(x => x.Id).ToList();
                if (user.Apps.Contains(0))
                {
                    currentApps.Add(0);
                }
                hasPermissionNode = _BaseService.Repository.Entities.Where(n => n.AppId != null && currentApps.Contains((int)n.AppId)).ToList();
