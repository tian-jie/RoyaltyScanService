...
namespace Innocellence.Web.Controllers
{
    public class PluginsController : BaseController<PluginModel, PluginView>
    {
        ...
        private string statusHtmlPath= "~/status.html";
        private string statusAspxPath= "~/status.aspx";
        ...
        {
            ViewBag.Status = GetServerStatus();
        ...
        {
           if (IsStatusFileThere()) { 

                return CheckStatus(Server.MapPath(statusHtmlPath)) +CheckStatus(Server.MapPath(statusAspxPath));
        ...
        private bool IsStatusFileThere()
        { 
            if (System.IO.File.Exists(Server.MapPath(statusHtmlPath)) && System.IO.File.Exists(Server.MapPath(statusAspxPath)))
            {
                return true;
            }
            else {
        ...
        private static string CheckStatus(string strPath)
        {
            using (FileStream file1 = new FileStream(strPath, FileMode.Open))
            {
                StreamReader rd = new StreamReader(file1);
        ...
        public JsonResult ChangeStatus(string type)
        {
            try
            {
                if (!string.IsNullOrEmpty(type)&&IsStatusFileThere())
                {
                    type = type.ToLower();
                    type = type.IndexOf("down") > -1 ? "up" : "down";                     
                    ChangeyFile(Server.MapPath(statusAspxPath), type);
                    ChangeyFile(Server.MapPath(statusHtmlPath), type);

                    return Json(new { returnCode = 200 }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { returnCode = 600 }, JsonRequestBehavior.AllowGet);
            ...
            {
                Logger.Error(ex);
                return Json(new{ returnCode = 500}, JsonRequestBehavior.AllowGet);
            }
        }

        private void ChangeyFile(string path,string type)
        {
            var toPath = path;
            var fromPath = path.Replace("status.", "status" + type+".");
        ...
        private void Uninstall(PluginDescriptor pluginDescriptor)
        {
            var systemName = pluginDescriptor.SystemName;
            //uninstall children plugins
            var plugsTemp = ((List<PluginDescriptor>)PluginManager.AllPlugins)
                                       .Where(a => a.Dependences.Contains(systemName)).ToList();
            if (plugsTemp.Any())
            {
                foreach (var p in plugsTemp)
                {
